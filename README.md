# Projet n°18 : Monitoring sonore en temps réel d’une rue via capteurs distribués
## _dbMeter-MA-Metier-P18_
### Samuel Theytaz, Mouldi Achouri, Nuno Ribeiro Pereira, Aurélien Nicollas


## Table des matières
1. [Présentation du projet](#présentation-du-projet)
2. [Raspberry Pi](#raspberry-pi)
   1. [Présentation des Raspberry Pi](#présentation-des-raspberry-pi)
   2. [Software intégré](#software-intégré)
      1. [Langage(s) de programmation utilisé(s)](#langages-de-programmation-utilisés)
      2. [Librairies utilisées](#librairies-utilisées)
3. [Réseau mis en place](#réseau-mis-en-place)
   1. [Connexion inter-Raspberry](#connexion-inter-raspberry)
   2. [Protocol(s) utilisé(s)](#protocols-utilisés)
   3. [Fonctionnement du réseau](#fonctionnement-du-réseau)
4. [Serveur Web](#serveur-web)
   1. [Hardware du serveur](#hardware-du-serveur)
   2. [Langage(s) de programmation utilisé(s)](#langages-de-programmation-utilisés-1)


## Présentation du projet

Le projet n°18 consiste à mettre en place des capteurs sonores dans la ville de Sainte-Croix afin d'y mesurer le niveau sonore<br>
Pour cela, nous ont étés mis à disposition des _Raspberry Pi 4_, équipés de capteurs sonores.<br>
Pour mettre en place ce projet, plusieurs étapes principales doivent être mises en place :<br>

1. L'installation du matériel
2. Le développement du système embarqué
3. La connexion entre chaque capteur et le serveur
4. La mise en place d'un serveur web
5. La récolte des données, ainsi que leur affichage

_Suite à venir_

## Raspberry Pi

### Présentation des Raspberry Pi

_Suite à venir_

### Software intégré

#### &nbsp;&nbsp;&nbsp;&nbsp;Langage(s) de programmation utilisé(s)
&nbsp;&nbsp;&nbsp;&nbsp;_Suite à venir_

#### &nbsp;&nbsp;&nbsp;&nbsp;Librairies utilisée(s)
&nbsp;&nbsp;&nbsp;&nbsp;_Suite à venir_

## Réseau mis en place

### Connexion inter-Raspberry

_Suite à venir_

### Protocol(s) utilisé(s)

_Suite à venir_

### Fonctionnement du réseau

_Suite à venir_

## Serveur Web

### Hardware du serveur

_Suite à venir_

### Langage(s) de programmation utilisé(s)

_Suite à venir_

Dépôt pour le projet 18, Monitoring du bruit dans en temps réel dans une rue de Ste-Croix